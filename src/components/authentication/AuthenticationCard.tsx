...
interface IAuthenticationCard {
    ...
    methods: {title: string, path: {d: string; fill: string}[], width: number, height: number; viewBox: string} [];
    authenticationBtn: string;
    link: string;
}

export const AuthenticationCard = ({textLevel, rightInterstData, methods, authenticationBtn, link}:IAuthenticationCard) => {
    return (
        <div className="p-4 bg-[#1c1c1e] rounded">
            <h2 className="text-xl text-white pb-8 border-b border-[#ffffff1a]">{textLevel}</h2>
            <div className="flex flex-col gap-4">
                <div className="flex flex-col gap-2">
                    <span className="text-sm text-white">{i18next.t("authentication.rightsAndInterests")}</span>
                ...
                <div className="flex gap-4">
                    {methods.map((method, index) => (
                        <div key={index} className="flex items-center gap-2 text-base text-white">
                            <Icon 
                                d={method.path}
                                width={method.width}
